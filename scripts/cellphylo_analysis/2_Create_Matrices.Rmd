...
cross-species_int <- cross_species_integration(matrix_path = "matrix/combined_by_orthology/matrix_combined", k_weight=100, print=TRUE)

#let's visualize the cross-species integration

#plot
p1 <- DimPlot(combined, reduction = "umap", group.by = "species_id")
#cells cluster by cell type?
p2 <- DimPlot(combined, reduction = "umap", group.by = "cell_type_id")
#cell types in each species present in the same clusters across species?
p3 <- DimPlot(combined, reduction = "umap", group.by = "cell_type_id", split.by = "species_id")
