...
We have provided this matrix in `scjackknife/contml_919cell_subset_20PC_mtx`. 

The above code produces a matrix where all.equal=TRUE with the provided matrix, but it is possible that machine-specific rounding errors may lead to identical()=FALSE.
...
We create it by removing unstable cells and singletons from the 919 cell cross-species integrated matrix. This leaves 54 cell type groups, each with 10 cells each. We only sample the cell labels from this matrix, not the values.
...
Since the scjackknife matrices are random, we have included the input as `scjackknife/input_scjack_540_TBE.tar.gz`
...
1. We created a template for the script that is executed for each contml run. This has been provided as `analysis/scripts/in_script_scjack_parallel_540_TBE.sh`

2. We made 500 copies of the template script. The script for this is provided as `analysis/scripts/make_reps_scjack_540_TBE.sh`.

3. Execute each scjackknife script in parallel. The script for this is `analysis/scripts/run_scjack_parallel_540_TBE.sh`.

We have included the scripts in `scjackknife/input_scjack_540_TBE/scjack_scripts.tar.gz` for reproducibility.

Now execute the scjackknife analysis on a cluster using the provided bash script `scripts/run_scjack_parallel_540_TBE.sh`.

This produced 500 outtrees representing a scjackknife tree inferred from each resampled 54 cell matrix. These trees have been provided as `analysis/scjackknife/output/raw/outtrees_scjack_540_TBE`
...
Moving forward, use the directories of `outtrees` that have had errored runs removed for downstream analyses. This directory has been provided as `output/output_scjack_540_TBE_processed/outtrees_scjack_540_TBE_processed`.
...
n_trees =439
matrix_dir_path = "scjackknife/output/output_scjack_540_TBE_processed/scjack_matrices_processed/"
tree_dir_path = "scjackknife/output/output_scjack_540_TBE_processed/outtrees_scjack_540_TBE_processed/"
...
These labelled trees are provided in:
`scjackknife/output/output_scjack_540_TBE_processed/outtrees_scjack_540_TBE_common_label`
`scjackknife/output/output_scjack_540_TBE_processed/outtrees_scjack_540_TBE_label`
