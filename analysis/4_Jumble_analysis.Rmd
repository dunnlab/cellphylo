...
The bash script `scripts/script_jumble_parallel_54.sh` was used to calculate jumble trees from a contml infile. It produces a random seed for contml to randomize the order of input taxa and runs `contml`. Running this script produces 100 jumble trees, from which the single tree with the best likelihood score is selected as the final output tree. This was performed 100 times to obtain 100 high-likelihood jumble trees for downstream analysis.
...
The infile is the infile created above from the 54 cell 20 PC matrix.
...
The output trees have been provided as `jumble/output/outtrees_54_jumble_search_processed.tar.gz`
`
