...
The jumble analysis was performed on the 54 cell 20 PC matrix (Fig. S1, step C1.2). See `2_Create_Matrices.Rmd`.  
...
```
*This infile has been provided in `jumble/input/infile_54_20PC`*. `contml` expects the input file to be named "infile", so rename this file when running.

## Infer jumble trees from contml runs

The bash script *`scripts/jumble/script_jumble_parallel_54.sh`* was used to infer jumble trees from a contml infile. It produces a random seed for contml to randomize the order of input taxa and runs `contml`. Running this script produces 100 jumble trees, from which the single tree with the best likelihood score is selected as the final output tree. This was performed 100 times to obtain 100 high-likelihood jumble trees for downstream analysis. The script *`scripts/jumble/run_jumble_parallel_54.sh`* was used to run `script_jumble_parallel_54.sh` in parallel on the cluster.
...
Infer the jumble trees from the infile for the 54 cell 20 PC matrix (Fig. S1, step C1.2)
...
*The processed output trees have been provided as `jumble/output/outtrees_54_jumble_search_processed.tar.gz`*
...
The tree with the minimum *unrounded* Kuhner-Felsenstein distance with all other "best trees" is outtree_54_jumble_search_1249177875.tre. Thus, this tree and its jumble scores was chosen to provide the topology of the tree in Figure 4. *We provide this tree at `analysis/jumble/output/outtree_54_jumble_search_1249177875.tre`.*
