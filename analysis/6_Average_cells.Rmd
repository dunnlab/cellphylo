...
The bash script `analysis/scripts/average/jumble/script_jumble_parallel_53mean_20PC_afterPCA.sh` was used to run contml. The script `analysis/scripts/average/jumble/run_jumble_parallel_53mean_20PC_afterPCA.sh` was used to run that script in parallel on a hpc cluster.
...
>>> PAUSED HERE TO PURSUE JUMBLE RUN <<<
We will plot scjack TBE values onto the best Jumble tree. It must share common labels with the scjackknife trees.

```{r}

tree <- read.tree("~/repo/aqhumor/AqHum_5Species/aqhumor_final/average/trees_ave/jumble/53mean_20PC_afterPCA/best_tree/RF_best_trees/outtree_53mean_20PC_afterPCA_jumble_search_1166074061.tre")
